const places = [
  {
    name: "Kalayaan Avenue, Diliman, Quezon City",
...
export function addressFromPin([lat, lng]) {
  const nearest = places.reduce((best, place) =>
    (lat - place.position[0]) ** 2 + (lng - place.position[1]) ** 2 <
    (lat - best.position[0]) ** 2 + (lng - best.position[1]) ** 2
      ? place
      : best,
  );
...
export function pinFromAddress(address) {
  ...
  const matched = places.find(
    (place) =>
      query.includes(place.name.split(",").slice(-1)[0].trim().toLowerCase()) ||
      query.includes(place.name.split(",")[0].toLowerCase()),
  );
  ...
  return [
    14.52 + (normalized % 1500) / 10000,
    120.96 + (Math.floor(normalized / 1500) % 1200) / 10000,
  ];
}
